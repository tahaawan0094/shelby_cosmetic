...
export default function VariantSelector({ shades = [], variants = [], sizeOptions = [], onVariantChange }) {
  ...
  return <div className="space-y-6 border-y border-neutral-200 py-6">
    ...
    {safeVariants.length > 0 && <div><p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em]">Shade</p><div className="flex flex-wrap gap-5">{safeVariants.map((variant) => <button type="button" key={variant.name} onClick={() => { setSelectedVariant(variant.name); onVariantChange?.(variant) }} title={variant.name} className="flex min-w-20 flex-col items-center gap-2 text-center"><span className={`h-12 w-12 rounded-full border-2 transition-shadow ${selectedVariant === variant.name ? 'border-brand ring-2 ring-rose-100' : 'border-white ring-1 ring-neutral-300'}`} style={{ backgroundColor: variant.value || '#d9a18c' }} /><span className="text-[11px] leading-tight text-neutral-700">{variant.name}</span></button>)}</div></div>}
